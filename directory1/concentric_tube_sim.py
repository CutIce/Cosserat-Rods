...
rod2_para ={
    ...
    "length": 0.5,
    "radius": 0.007,
    "start": np.array([0.0, 0.0, rod1_para["length"]]),
    "direction": np.array([0.0, 0.0, 1.0]),
    "normal": np.array([0.0, 1.0, 0.0]),
    "density": 2e3,
    "nu": 0,
...
# origin_force = np.array([0.0, 0.1, 0.0])
# end_force = np.array([0.2, 0.0, 0.0])
...
gravitational_acc = np.array([0, 0, 9.80665])
...
tube.add_forcing_to(rod2).using(
    ...
    end_force_mag=5e-2,
    ramp_up_time=0.2,
    tangent_direction=rod1_para["direction"],
    normal_direction=rod1_para["normal"],
)
...
tube.dampen(rod1).using(
    ...
    damping_constant=rod1_para["damping_constant"],
    time_step=dt,
)
tube.dampen(rod2).using(
    AnalyticalLinearDamper,
    damping_constant=rod2_para["damping_constant"],
    time_step=dt,
)
